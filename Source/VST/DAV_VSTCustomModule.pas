...
uses
  ...
  DAV_VSTShellPlugins, DAV_VSTOfflineTask, AnsiStrings;
  ...
  begin
   ...
   try
    if GetHostProductString(Text)
     then Result := SysUtils.StrPas(Text)
     else Result := 'Unknown';
...
begin
 ...
 if Assigned(ptr) then
  begin
   if FProductName <> ''
    then SysUtils.StrPCopy(ptr, FProductName)
    else SysUtils.StrPCopy(ptr, FEffectName);
   Result := 1;
  end;
end;
